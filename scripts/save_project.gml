...
buffer_write(save_buffer,buffer_u8,!parsingaudio); //audio parsed
...
for (i = 0; i < ds_list_size(layer_list); i++)
    ...
    buffer_write(save_buffer,buffer_s32,ds_list_size(layer)/3);
    for (j = 0; j < ds_list_size(layer); j+=3)
        ...
        buffer_write(save_buffer,buffer_s32,tempframe);
        buffer_write(save_buffer,buffer_s32,buffer_get_size(tempbuffer));
        ...
        buffer_write(save_buffer,buffer_s32,ds_list_find_value(tempinfolist,0));
        buffer_write(save_buffer,buffer_s32,ds_list_find_value(tempinfolist,2));
        }
    }
...
if (song)
    ...
    buffer_write(save_buffer,buffer_s32,songfile_size);
    ...
    if (parsingaudio)
        ...
        parsinglistsize = ds_list_size(audio_list);
            ...
            {
            buffer_write(save_buffer,buffer_s32,ds_list_find_value(audio_list,i));
            }
        }
    }
