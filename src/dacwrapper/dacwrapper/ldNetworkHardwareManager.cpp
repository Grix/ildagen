...
ldNetworkHardwareManager::ldNetworkHardwareManager(QObject *parent)
...
ldNetworkHardwareManager::DeviceBufferConfig ldNetworkHardwareManager::getBufferConfig()
{
     //QMutexLocker locker(&m_mutex);
     return *m_currentBufferConfig;
}

void ldNetworkHardwareManager::setExplicitActiveDevice(int index)
{
...
void ldNetworkHardwareManager::sendData(LaserdockSample* samples, uint32_t count)
{
    //QMutexLocker locker(&m_mutex);
    ...
    if (m_networkHardwares.size() == 1) {
        ldNetworkHardware* networkHardware = m_networkHardwares.at(0).get();
        if (networkHardware->isActive()) {
            networkHardware->send_samples(samples, count);
        } // end isactive
    ...
    else // multiple cubes get data fed to all
    {
        ...
        while (networkHardwareIt != m_networkHardwares.end()) {
            ...
            bool b = networkHardware->send_samples(samples, count);
...
std::vector<ldNetworkHardware*> ldNetworkHardwareManager::devices() const
{
    std::vector<ldNetworkHardware*> devices;
...
bool ldNetworkHardwareManager::hasActiveDevices() const
{
    ...
    auto it = std::find_if(m_networkHardwares.begin(), m_networkHardwares.end(),
            ...
            return (networkHardware->isActive() && networkHardware->status() == Status::INITIALIZED);
    });
    ...
    if (m_initializingnetworkHardwares.size()>0){ // any network devices that are still initializing?
        ...
        while(initnetworkHardwareIt != m_initializingnetworkHardwares.end()) {
            ...
            if (networkHardware->params.device->status() == LaserdockNetworkDevice::Status::INITIALIZED){
                    if (networkHardware->status() == Status::INITIALIZED){
...
W_OBJECT_IMPL(ldNetworkHardwareManager)