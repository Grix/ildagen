...
W_REGISTER_ARGTYPE(uint8_t)
W_REGISTER_ARGTYPE(int8_t)
W_REGISTER_ARGTYPE(uint16_t)
W_REGISTER_ARGTYPE(uint32_t)
...
{
    W_OBJECT(LaserdockNetworkDevice)
public:
...
public:
    ...
    void DeviceReady()  W_SIGNAL(DeviceReady)
    // sent out when device is ready to be authenticated
    void DeviceNeedsAuthenticating() W_SIGNAL(DeviceNeedsAuthenticating)
    // sent when there is a security request response available
    void SecurityResponseReceived(bool success, QByteArray response_data) W_SIGNAL(SecurityResponseReceived, success, response_data)

    // new signal events triggered when data changes from this lasercube device
    void DACRateUpdated(uint32_t dacrate) W_SIGNAL(DACRateUpdated, dacrate)
    void SampleBufferFreeUpdated(uint16_t buffer_free_samples) W_SIGNAL(SampleBufferFreeUpdated, buffer_free_samples)
    void SampleBufferSizeUpdated(uint16_t buffer_size_samples) W_SIGNAL(SampleBufferSizeUpdated, buffer_size_samples)
    void BatteryPercentUpdated(uint8_t battery_percent) W_SIGNAL(BatteryPercentUpdated, battery_percent)
    void TemperatureUpdated(int8_t temperature_degc) W_SIGNAL(TemperatureUpdated, temperature_degc)
    void ConnectionTypeUpdated(int connection_type) W_SIGNAL(ConnectionTypeUpdated, connection_type)
    void SerialNumberUpdated(QString serial_num_str) W_SIGNAL(SerialNumberUpdated, serial_num_str)
    void ModelNameUpdated(QString model_name_str) W_SIGNAL(ModelNameUpdated, model_name_str)
    void ModelNumberUpdated(uint8_t model_num) W_SIGNAL(ModelNumberUpdated, model_num)
    void UniqueIdUpdated(QString unique_id_str) W_SIGNAL(UniqueIdUpdated, unique_id_str)
    void IPAddressUpdated(QString ip_addr_str) W_SIGNAL(IPAddressUpdated, ip_addr_str)
     
public:// slots:
    
    // perform a security request to this device
    void SecurityRequest(QByteArray request); W_SLOT(SecurityRequest)
    // set this device as authenticated or not.
    void DeviceAuthenticated(bool state); W_SLOT(DeviceAuthenticated)
    ...
    void SetModelInfo(uint8_t region, uint8_t model_num, QString model_str); W_SLOT(SetModelInfo)
...
private :
    ...
    void SamplesReady() W_SIGNAL(SamplesReady)
    void RequestBufferSize() W_SIGNAL(RequestBufferSize)

    private:// slots:

    void processSamples(); W_SLOT(processSamples, W_Access::Private)
    void readPendingDataResponses(); W_SLOT(readPendingDataResponses, W_Access::Private)
    void readPendingCommandResponses(); W_SLOT(readPendingCommandResponses, W_Access::Private)
    void process(); W_SLOT(process, W_Access::Private)
