function layer_duplicate() {
	...
	var t_new_element_list = ds_list_create();
	for (t_i = 0; t_i < ds_list_size(t_layer[| 1]); t_i++)
	{
		var t_new_element = ds_list_create();
		...
		var t_info_list = ds_list_create();
		ds_list_add(t_info_list, t_layer[| 1][| t_i][| 2][| 0]);
		ds_list_add(t_info_list, -1);
		ds_list_add(t_info_list, t_layer[| 1][| t_i][| 2][| 2]);
		ds_list_add(t_info_list, create_checkpoint_list(t_framebuffer));
		ds_list_add(t_new_element, t_info_list);
	...
	controller.el_id++;
	var t_new_dac_list = ds_list_create();
	for (t_i = 0; t_i < ds_list_size(t_layer[| 5]); t_i++)
	{
		var t_new_dac = ds_list_create();
	...
	var t_new_undo_list = ds_list_create();
