function refresh_timeline_surface() {
	...
	{
	    timeline_surf = surface_create(power(2, ceil(log2(tlw+512))), power(2, ceil(log2(view_hport[1]/controller.dpi_multiplier))));
	...
	if (tlx+tlzoom-t_tlx > -50/tlwdivtlzoom)
	{
		if (!surface_exists(timeline_surf_temp))
			timeline_surf_temp = surface_create(power(2, ceil(log2(tlw+512))), power(2, ceil(log2(view_hport[1]/controller.dpi_multiplier))));
		...
		surface_set_target(timeline_surf_temp);
		    ...
		    for (i = 0; i < ds_list_size(layer_list);i++)
		    {
		        ...
		        if (ypos_perm > -48) and (ypos_perm < lbsh)
		        {
					...
					draw_rectangle(-1,ypos_perm,t_tlw+1,ypos_perm+47,1);
		        ...
		        for (j = 0; j < ds_list_size(envelope_list); j++)
		        {
		            if (ypos_perm > -64) and (ypos_perm < lbsh)
		            {
		                ...
		                if (!hidden)
		                {
		                    draw_rectangle(-1,ypos_perm,t_tlw+1,ypos_perm+63,0);
		                    draw_set_colour(c_ltgray);
		                    draw_line(-1,ypos_perm+32,t_tlw+1,ypos_perm+32);
		                    draw_set_colour(c_black);
		                    draw_rectangle(-1,ypos_perm,t_tlw+1,ypos_perm+63,1);
		                }
		                else
		                {
		...
		surface_set_target(timeline_surf_audio_temp);
			...
			draw_rectangle(-1,tlh,t_tlw+1,tlh+14,1);
