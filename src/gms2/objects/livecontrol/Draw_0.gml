...
{	
	...
	draw_text_transformed(view_wport[4] - 16,7,"FPS: "+string(controller.projectfps/controller.fpsmultiplier),controller.dpi_multiplier, controller.dpi_multiplier, 0);
	if (playing && (fps != controller.projectfps/controller.fpsmultiplier) && controller.laseron)
	{
	    draw_set_color(c_red);
	    draw_text_transformed(view_wport[4] - 16,24*controller.dpi_multiplier,"Warning: Dropping frames. Actual FPS: "+string(fps),controller.dpi_multiplier, controller.dpi_multiplier, 0);
	}
...
else if (view_current == 3)
{
	...
	draw_line(0, t_ypos+t_height-1, t_width, t_ypos+t_height-1);
