...
else if (moving == 2)
{
	...
	controller.tooltip = "Click and drag the projector to change the position of the simulated projector for the selected layer in the timeline mode.";
    ...
    &&  (mouse_x == clamp(mouse_x, x, x+list_width))
{
    ...
    for (i = 0; i < ds_list_size(seqcontrol.layer_list); i++)
    {
        ...
        if (t_item == t_item_mouseover)
        {
            ...
            controller.tooltip = "Click to view or set the position of the projector in the simulated preview of this layer in the timeline mode.";
            if (mouse_check_button_pressed(mb_left))
            {
                selected = i;
				if (surface_exists(surf_projectorlist))
