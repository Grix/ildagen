...
{
    controller.tooltip = "Changes secondary color (Sliders represent red, green and blue)\n->Right click to enter a precise value";
} 
...
if (moving == 4)
{
    ...
    if (mouse_y-y < 47+9)
		controller.color2 = make_color_rgb(255, floor((mouse_x-x)/8)*51, 0);
	else if (mouse_y-y < 47+18)
		controller.color2 = make_color_rgb(0, 255, floor((mouse_x-x)/8)*51);
	else if (mouse_y-y < 47+27)
		controller.color2 = make_color_rgb(floor((mouse_x-x)/8)*51, 0, 255);
	else if (mouse_y-y < 47+36)
		controller.color2 = make_color_rgb(255 - floor((mouse_x-x)/8)*51, 255 - floor((mouse_x-x)/8)*51, 255 - floor((mouse_x-x)/8)*51);
    moving = 0;
    update_colors();
}
else
